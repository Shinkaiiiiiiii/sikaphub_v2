        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8 relative z-20">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <?php if (empty($jobs)): ?>
                    <div class="bg-white rounded-2xl p-10 border border-slate-200 shadow-sm text-center">
                        ...
                        <h3 class="text-lg font-bold text-slate-700 mb-1">No matches yet</h3>
                        <p class="text-sm text-slate-400">No jobs match your profile yet. Check back soon!</p>
                    </div>
                    <?php else: ?>
                    ...
                    <div class="job-card bg-white rounded-2xl p-6 border border-slate-200 shadow-sm flex flex-col sm:flex-row gap-6 relative">
                        <div class="flex-grow">
                            <div class="flex justify-between items-start mb-2">
                                <div>
                                    <h3 class="text-lg font-bold text-slate-900 group-hover:text-primary transition-colors cursor-pointer"><?php echo htmlspecialchars($job['job_title'] ?? ''); ?></h3>
                                    <p class="text-sm font-semibold text-primary"><?php echo htmlspecialchars($job['company_name'] ?? ''); ?></p>
                                </div>
                                <div class="flex flex-col items-center justify-center bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-2 min-w-[80px]">
                                    ...
                                    <span class="text-xl font-extrabold text-indigo-700"><?php echo htmlspecialchars($job['match_percentage'] ?? ''); ?>%</span>
                                </div>
                            </div>
                            ...
                            <div class="flex flex-wrap gap-2 mb-4 mt-3">
                                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-md text-xs font-medium bg-slate-100 text-slate-600">
                                    ...
                                    <?php echo htmlspecialchars($job['municipality_name'] ?? ''); ?>
                                </span>
                                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-md text-xs font-medium bg-slate-100 text-slate-600">
                            ...
                            <div class="flex items-center justify-between mt-5 pt-4 border-t border-slate-100">
                                <span class="text-xs font-medium text-slate-400">Posted <?php echo htmlspecialchars(date('M d, Y', strtotime($job['date_posted']))); ?></span>
                                <div class="flex items-center gap-2">
                                    <a href="/sikaphub_v2/job/view?id=<?php echo $job['job_id']; ?>" class="text-sm font-bold text-slate-700 border border-slate-200 hover:border-primary hover:text-primary px-5 py-2 rounded-lg transition-colors">View Job</a>
                                    <form method="POST" action="/sikaphub_v2/apply">
                                        <input type="hidden" name="job_id" value="<?php echo $job['job_id']; ?>">
                                        <input type="hidden" name="csrf_token" value="<?php echo $_SESSION['csrf_token']; ?>">
                                        <button type="submit" class="text-sm font-bold text-white bg-gradient-to-r from-primary to-secondary hover:from-indigo-500 hover:to-violet-500 px-5 py-2 rounded-lg transition-all shadow-sm">1-Click Apply</button>
                                    </form>
                                </div>
                            </div>
                        </div>
